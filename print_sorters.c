 ...
 * Return: Number of chars printed.
 */
int print_numba(int is_neg, int typ, char buffer[],
	int flags, int width, int precision, int size)
{
	int length = BUFF_SIZE - typ - 1;
	char lado = ' ', plus_ch = 0;
	...
	return (write_num(typ, buffer, flags, width, precision,
		length, lado, plus_ch));
}
 ...
 */
int write_num(int idex, char buffer[],
	...
	int length, char lado, char plus_c)
{
	...
	if (prec == 0 && idex == BUFF_SIZE - 2 && buffer[idex] == '0')
		buffer[idex] = lado = ' '; /* width is displayed with padding ' ' */
