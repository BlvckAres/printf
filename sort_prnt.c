#include "main.h"
/**
 * sort_prnt - Prints an argument based on its arrg
	...
	int flags, int width, int precision, int size)
{
	...
	for (i = 0; typ_arrgs[i].typ != '\0'; i++)
		if (typ[*ndx] == typ_arrgs[i].typ)
			return (typ_arrgs[i].fn(list, buffer, flags, width, precision, size));

	if (typ_arrgs[i].typ == '\0')
	{
		if (typ[*ndx] == '\0')
			return (-1);
		un_length += write(1, "%%", 1);
